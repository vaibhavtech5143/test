  ...
  const NavContent = () => {
    ...
    return (<section className='flex h-full flex-col gap-6 pt-16'>
      {sidebarLinks.map((item) => {
        const isActive = (pathName && (pathName.includes(item.route) && item.route.length > 1)) || pathName === item.route;
        return (
          <SheetClose asChild key={item.route}>
            <Link href={item.route} className={`${isActive ? 'primary-gradient rounded-lg text-light-900' : 'text-dark300_light900'} justify-start p-4 flex items-center gap-4 bg-transparent`}>  
